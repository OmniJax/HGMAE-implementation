...
class HGMAETrainer(BaseFlow):
    def __init__(self, args):
        ...
        self.model = build_model(self.model).build_model_from_args(self.args, self.hg, self.meta_paths_dict)
        print("build_model_finish")
        self.model = self.model.to(self.device)
        self.evaluator = self.task.get_evaluator('f1')

        self.optimizer = optim.Adam(self.model.parameters(), lr=args.mae_lr, weight_decay=args.l2_coef)
        self.scheduler = optim.lr_scheduler.ExponentialLR(self.optimizer, gamma=args.scheduler_gamma)
        self.patience = args.patience
        self.max_epoch = args.mae_epochs
        self.epoch = 0
    ...
    def preprocess(self):
        super(HGMAETrainer, self).preprocess()
    ...
    def _full_train_step(self):
        ...
        h_dict = self.model.input_feature()
        loss = self.model(self.hg, h_dict, trained_mp2vec_feat_dict=None, epoch=self.epoch)
        self.epoch += 1
        loss.backward()
        self.optimizer.step()
        self.scheduler.step()
